  ...
  const answer = useCallback(
    (q: string) => {
      ...
      if (/^(hi|hello|hey|namaste|kem cho|good (morning|evening|afternoon)|wake up)/.test(t)) {
        ...
        speak(
          "I am Friday — Technova Tech's operations AI. I watch the agency pipeline, the agents, and the money, and I report only the truth to you, Boss.",
        )
      } else if (/my name|what.*name|maru naam|mara naam|naam su|naam khabar|naam khbar|who am i|hu kon|hu kaun/.test(t)) {
        const boss = data.brain?.boss
        speak(
          boss
            ? `You're ${boss.name}, Boss${boss.role ? " — " + boss.role + " of Technova Tech" : ""}. Of course I know who I work for.`
            : "You're the boss — the founder of Technova Tech.",
        )
